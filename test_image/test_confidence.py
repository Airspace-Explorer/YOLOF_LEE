...
for index, img_path in tqdm(enumerate(test_file), total = len(test_file)):
    ...
    if len(labels) != 0:
        ...
        for label, score, bbox in zip(labels, scores, boxes):
            ...
            img_result = cv2.polylines(img_result, [points], isClosed=True, color=(0, 0, 255), thickness=3)
            class_name = class_names[label]  # Assuming label is the class identifier
            text = f"Class: {class_name}, Confidence: {score:.2f}"
            cv2.putText(img_result, text, (x_min, y_min - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
